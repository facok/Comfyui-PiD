...
from pid_core._ext.imaginaire.utils.easy_io import easy_io
...
class ModelWrapper(Stateful):
    """Wrapper for model state dict handling with DCP (Distributed Checkpoint).

    Usage flow: state_dict() -> dcp.load() -> load_state_dict()

    When load_ema_to_reg=True, the three steps work together to load EMA weights from
    checkpoint into the regular model:
      1. state_dict():      prepare keys so dcp.load() reads EMA weights from checkpoint
      2. dcp.load():        fills the state_dict with checkpoint data (in-place)
      3. load_state_dict(): remap loaded EMA weights back to regular model keys, then apply
    """
    ...
    def __init__(self, model: Union[nn.Module, List[nn.Module]], load_ema_to_reg: bool = False):
        ...
        if self.load_ema_to_reg:
            assert isinstance(model, ImaginaireModel), (
                f"replicate_ema_to_reg requires an ImaginaireModel, got {type(model).__name__}"
            )

    def state_dict(self) -> Dict[str, Any]:
        """Build a state_dict whose keys will be matched against checkpoint keys by dcp.load().

        When load_ema_to_reg=True, we manipulate the keys so that dcp.load() reads the
        checkpoint's net_ema.* weights (instead of net.*):

        Case A - ema.enabled=False (model has no EMA module, only net.*):
          Rename net.* -> net_ema.* so dcp.load() matches checkpoint's net_ema.* keys.
          Result after dcp.load(): state_dict has net_ema.* filled with checkpoint EMA weights.

        Case B - ema.enabled=True (model has both net.* and net_ema.*):
          Drop net.* keys entirely, keep net_ema.* (and other keys like fake_score.*).
          Result after dcp.load(): net_ema.* filled from checkpoint; other keys (e.g. fake_score.*)
          loaded if present in checkpoint, otherwise unchanged.
        """
        _state_dict = {k: v for sd in map(get_model_state_dict, self.model) for k, v in sd.items()}
        if self.load_ema_to_reg:
            if not self.model[0].config.ema.enabled:
                # Case A: model only has net.* -> rename to net_ema.* to match checkpoint EMA keys.
                # Auxiliary keys (aux_losses.*, teacher_align_loss.*, etc.) are passed through as-is.
                all_keys = list(_state_dict.keys())
                for k in all_keys:
                    if k.startswith("net."):
                        _state_dict[k.replace("net.", "net_ema.", 1)] = _state_dict.pop(k)
            else:
                # Case B: model has net.* + net_ema.* -> drop net.*, only load net_ema.* from ckpt
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        """Apply the checkpoint-loaded state_dict back to the model.

        When load_ema_to_reg=True, remap the loaded EMA weights to regular model keys:

        Case A - ema.enabled=False:
          state_dict has net_ema.* (loaded from checkpoint) -> rename back to net.* for the model.

        Case B - ema.enabled=True:
          state_dict has net_ema.* (loaded from checkpoint) -> clone each net_ema.* as net.*
          so the regular model gets the same EMA weights. net_ema.* is also kept.
        """
        if hasattr(self.model[0].config, "lora_config") and self.model[0].config.lora_config.enabled:
        ...
        if self.load_ema_to_reg:
            if not self.model[0].config.ema.enabled:
                # Case A: rename net_ema.* back to net.* so set_model_state_dict can apply them.
                # Auxiliary keys (aux_losses.*, teacher_align_loss.*, etc.) are passed through as-is.
                all_keys = list(state_dict.keys())
                for k in all_keys:
                    if k.startswith("net_ema."):
                        state_dict[k.replace("net_ema.", "net.", 1)] = state_dict.pop(k)
            else:
                # Case B: clone net_ema.* -> net.* so regular model gets EMA weights too.
                # Auxiliary keys are kept unchanged.
                log.warning("EMA is enabled, will load EMA weights to regular model weights")
                all_keys = list(state_dict.keys())
                for k in all_keys:
                    if k.startswith("net_ema."):
                        state_dict[k.replace("net_ema.", "net.", 1)] = torch.clone(state_dict[k])
...
class OptimizerWrapper(Stateful):
    """
    Wrapper for optimizer state dict handling with DCP (Distributed Checkpoint).

    Note: For multi-optimizer models (e.g., distillation), use DistillationCheckpointer
    from pid_core._src.checkpointer.dcp_distill which has fallback handling for
    FSDP2 parameter ID mismatches.
    """
...
class DistributedCheckpointer(AbstractCheckpointer):
    ...
    def keys_to_resume_during_load(self) -> Tuple[Set, Union[str, None]]:
        ...
        is_self_resume = latest_checkpoint_file is not None
        return set(resume_keys), checkpoint_path, is_self_resume
    ...
    def load(
        ...
        resume_keys, checkpoint_path, is_self_resume = self.keys_to_resume_during_load()
        ...
        if checkpoint_path is not None and not checkpoint_path.endswith(".pth"):
            ...
            for key in resume_keys:
                ...
                if key == "model":
                    ...
                    # Note: For multi-optimizer models (e.g., distillation), use DistillationCheckpointer
                    # from pid._src.checkpointer.dcp_distill instead.
                    ...
                    # Note: For multi-scheduler models (e.g., distillation), use DistillationCheckpointer
                    # from pid._src.checkpointer.dcp_distill instead.
    ...
    def save(
        ...
        Note: For multi-optimizer models (e.g., distillation), use DistillationCheckpointer
        from pid_core._src.checkpointer.dcp_distill instead.
