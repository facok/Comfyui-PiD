...
from pid_core._src.models.pid_model import PidModel, PidModelConfig
...
class PidDistillModel(PidModel):
    ...
    def _get_t_list(self, device, num_steps: Optional[int] = None) -> torch.Tensor:
        ...
        _dtype = self.tensor_kwargs.get("dtype", torch.float32)
        if self.config.student_t_list is not None:
            full_t = torch.tensor(self.config.student_t_list, device=device, dtype=_dtype)
    ...
    def generate_samples_from_batch(
        ...
        **kwargs,
        ...
        if "degrade_sigma" not in data_batch and "LQ_latent" in data_batch:
            ...
            data_batch["degrade_sigma"] = torch.zeros(B, device=data_batch["LQ_latent"].device, dtype=_dtype)
        ...
        if isinstance(sigma_val, torch.Tensor):
            degrade_sigma_tensor = sigma_val.to(device="cuda", dtype=_dtype).reshape(-1)
            ...
            degrade_sigma_tensor = torch.tensor(sigma_val, device="cuda", dtype=_dtype)
            assert degrade_sigma_tensor.shape == (B,), (
                f"data_batch['degrade_sigma'] expected length {B}, got {len(sigma_val)}"
            )
        else:
            degrade_sigma_tensor = torch.full((B,), float(sigma_val), device="cuda", dtype=_dtype)

        gen = torch.Generator(device="cuda").manual_seed(int(seed))
        noise = torch.randn(B, 3, img_h, img_w, device="cuda", dtype=_dtype, generator=gen)
        ...
        if effective_steps == 1:
            t_student = torch.full((B,), self.config.student_timestep, device="cuda", dtype=_dtype)
